    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          ...
          'Fractal dimension D=log(20)/log(3) ≈ 2.7268 | Density ρ(n)=(20/27)^n',
          'Optics: I_out = I_in/(1+k*A), T=e^(-μ*L_opt), reflections ≈ 3^n',
...
function BulletItem({ label, description }: Bullet) {
  return (
    <ReactNative.View style={{ flexDirection: 'row', marginTop: 10 }}>
      <ReactNative.Text style={{ color: '#2b8cf6', marginRight: 8 }}>-</ReactNative.Text>
      <ReactNative.View style={{ flex: 1 }}>
        <ReactNative.Text style={{ color: '#f6f8ff', fontWeight: '700', marginBottom: 2 }}>{label}</ReactNative.Text>
        <ReactNative.Text style={{ color: 'rgba(255,255,255,0.75)' }}>{description}</ReactNative.Text>
      </ReactNative.View>
    </ReactNative.View>
  )
}
...
function QuizCard({ quiz }: { quiz: Quiz }) {
  ...
  return (
    <ReactNative.View style={{
      ...
      <ReactNative.Text style={{ color: '#f6f8ff', fontWeight: '800', fontSize: 16, marginBottom: 10 }}>Quick quiz</ReactNative.Text>
      <ReactNative.Text style={{ color: 'rgba(255,255,255,0.85)', marginBottom: 12 }}>{quiz.question}</ReactNative.Text>
      {quiz.options.map((opt, idx) => {
        const isSelected = selected === idx
        const bg = isSelected ? 'rgba(43,140,246,0.18)' : 'rgba(255,255,255,0.06)'
        const border = isSelected ? 'rgba(43,140,246,0.6)' : 'rgba(255,255,255,0.08)'
        return (
          ...
          >
            <ReactNative.Text style={{ color: '#f6f8ff' }}>{opt}</ReactNative.Text>
          </ReactNative.TouchableOpacity>
        )
      })}
      <ReactNative.TouchableOpacity
      ...
      >
        <ReactNative.Text style={{ color: '#f6f8ff', fontWeight: '700' }}>Check answer</ReactNative.Text>
      </ReactNative.TouchableOpacity>
      ...
      {submitted ? (
        <ReactNative.View style={{ marginTop: 12 }}>
          ...
          <ReactNative.Text style={{ color: 'rgba(255,255,255,0.75)', marginTop: 6 }}>{quiz.explanation}</ReactNative.Text>
        </ReactNative.View>
      ) : null}
    </ReactNative.View>
  )
...
export default function DetailScreen() {
  ...
  if (!data) {
    return (
      <ReactNative.View style={{ flex: 1, backgroundColor: '#0c1520', alignItems: 'center', justifyContent: 'center' }}>
        <ReactNative.Text style={{ color: '#f6f8ff', marginBottom: 12 }}>Page not found.</ReactNative.Text>
        <ReactNative.TouchableOpacity onPress={() => router.back()} style={{ paddingHorizontal: 14, paddingVertical: 10, backgroundColor: '#2b8cf6', borderRadius: 10 }}>
          <ReactNative.Text style={{ color: '#fff', fontWeight: '700' }}>Go back</ReactNative.Text>
        </ReactNative.TouchableOpacity>
      </ReactNative.View>
    )
  }
  ...
  return (
    <ReactNative.ScrollView style={{ flex: 1, backgroundColor: '#0c1520' }} contentContainerStyle={{ paddingBottom: 32 }}>
      <ReactNative.View style={{ paddingHorizontal: 16, paddingTop: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <ReactNative.Text style={{ color: '#f6f8ff', fontWeight: '800' }}>Menger Sponge</ReactNative.Text>
      ...
      <ReactNative.View style={{ padding: 16 }}>
        ...
        <ReactNative.Text style={{ color: '#f6f8ff', fontSize: 22, fontWeight: '800', marginBottom: 8 }}>{data.title}</ReactNative.Text>
        <ReactNative.Text style={{ color: 'rgba(255,255,255,0.78)', lineHeight: 22 }}>{data.intro}</ReactNative.Text>

        {data.sections.map((section) => (
          <ReactNative.View key={section.heading} style={{ marginTop: 18 }}>
            <ReactNative.Text style={{ color: '#2b8cf6', fontWeight: '800', marginBottom: 8 }}>{section.heading}</ReactNative.Text>
            <ReactNative.Text style={{ color: 'rgba(255,255,255,0.78)', lineHeight: 22 }}>{section.body}</ReactNative.Text>
          </ReactNative.View>
        ))}
        ...
        {data.highlight ? (
          <ReactNative.View style={{
            ...
            <ReactNative.Text style={{ color: '#f2f6ff', fontWeight: '800', marginBottom: 6 }}>Did you know?</ReactNative.Text>
            <ReactNative.Text style={{ color: 'rgba(255,255,255,0.78)' }}>{data.highlight}</ReactNative.Text>
          </ReactNative.View>
        ) : null}
